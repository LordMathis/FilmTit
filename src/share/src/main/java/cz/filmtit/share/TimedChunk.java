...
public class TimedChunk extends Chunk implements com.google.gwt.user.client.rpc.IsSerializable,
    ...
    /**
     * Joins the chunk with a different one if they are from one split subtitle
     * item in order to be exported to a file.
     *
    ...
    public StringBuilder getFileForm(FileType type, int order, double fps) {
        switch (type) {
            case SRT:
                return getSrtForm(order, fps);
            case SUB:
                return getSubForm(fps);
            case TXT:
                return new StringBuilder(this.getSurfaceForm()).append("\n");
        }
     ...
     * @param includeMillis Flag if the millisecond should include or
     * disregarded
    ...
    /**
     * Sets the subtitle item id if it has not been set before, it throws an
     * excpetion otherwise.
     *
     * @param id Subtitle item ID
     */
    ...
    /**
     * Sets the document ID if it has not been set before, otherwise it throws
     * an excpetion.
     *
     * @param documentId Document ID
     */
    ...
    /**
     * Comparing according to their order in the file/movie - by startTime,
     * endTime and partNumber, respectively. (returns 0 iff all these three are
     * the same)
     */
    @Override
    ...
    /**
     * When comparing two TimedChunks, equal iff their proper compareTo returns
     * 0.
     */
    @Override
